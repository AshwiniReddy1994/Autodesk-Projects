...
def LogmanCreation(WindowsInstance, HostName, UserName, PassWord, EndPoint):
    ...
    try:
        ...
        CreationOutput = 'Logman ' + HostName + '_' + PresentDate + '_000001' + '.blg' + ' ' + " is Successfully Created under " + "C:\\PerfMonLogs" + "\n\n"
...
def DiskReadDetails(WindowsInstance, HostName, UserName, PassWord, EndPoint):

    try:
        PsScript = '(Get-counter "\Process(*)\IO Read Bytes/sec").countersamples | sort cookedvalue | Select-MyObject @{Name="ProcessName";e={($_.InstanceName)}},@{Name="ReadLatency(ms)";e={($_.CookedValue)}} -First 5 | Format-Table'

        Info = ((JeaSession(WindowsInstance, HostName, UserName, PassWord, EndPoint, PsScript)).std_out).strip()
...
def DiskWriteDetails(WindowsInstance, HostName, UserName, PassWord, EndPoint):

    try:
        PsScript = '(Get-counter "\Process(*)\IO Write Bytes/sec").countersamples | sort cookedvalue | Select-MyObject @{Name="ProcessName";e={($_.InstanceName)}},@{Name="WriteLatency(ms)";e={($_.CookedValue)}} -First 5 | Format-Table'
...
def AverageDiskReadAndWriteDetails(WindowsInstance, HostName, UserName, PassWord, EndPoint):
    try:
        PsScript = 'Get-WmiObject -class Win32_PerfFormattedData_PerfDisk_PhysicalDisk | Select-MyObject Name, AvgDisksecPerWrite, AvgDisksecPerRead, DiskWritesPersec, DiskReadsPersec | Format-Table @{Expression={$_.Name}; Label="DiskName" }, @{Expression={$_.AvgDisksecPerWrite * 1000}; Label="WriteLatency(ms)"}, @{Expression={$_.AvgDisksecPerRead * 1 }; Label="ReadLatency(ms)"}, @{Expression={$_.DiskWritesPersec * 1}; Label="Write(IOPS)"}, @{Expression={$_.DiskReadsPersec * 1}; Label="Read(IOPS)" }'
...
def HealthCheck(WindowsInstance, HostName, UserName, PassWord, EndPoint):
    ...
    try:
        ...
        PageFileInfoScript = 'Get-wmiobject -class "Win32_PageFileUsage" | Format-List Name,@{Name="BaseSize";e={$_.AllocatedBaseSize}},CurrentUsage,PeakUsage | Out-String -Width 200'
        ...
        PageFileInfo = ((JeaSession(WindowsInstance, HostName, UserName, PassWord, EndPoint, PageFileInfoScript)).std_out).strip()
...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            LogmanCreationStorage, DiskReadStorage, DiskWriteStorage, AverageReadandWriteStore, HealthCheckStorage = Queue(), Queue(), Queue(), Queue(), Queue()
            Thread(target = MainThread, args=(LogmanCreation, FirstWindowsInstance, HostName, UserName, PassWord, EndPoint, LogmanCreationStorage)).start()
            Thread(target = MainThread, args=(DiskReadDetails, SecondWindowsInstance, HostName, UserName, PassWord, EndPoint, DiskReadStorage)).start()
            Thread(target = MainThread, args=(DiskWriteDetails, ThirdWindowsInstance, HostName, UserName, PassWord, EndPoint, DiskWriteStorage)).start()
            Thread(target = MainThread, args=(AverageDiskReadAndWriteDetails, FourthWindowsInstance, HostName, UserName, PassWord, EndPoint, AverageReadandWriteStore)).start()
            Thread(target = MainThread, args=(HealthCheck, FifthWindowsInstance, HostName, UserName, PassWord, EndPoint, HealthCheckStorage)).start()
            
            LogmanFinalOutput = LogmanCreationStorage.get()
            DiskReadOutput = DiskReadStorage.get()
            DiskWriteOutput = DiskWriteStorage.get()
            AverageDiskRedandWriteOutput = AverageReadandWriteStore.get()
            HealthCheckComments = HealthCheckStorage.get()
            
            CommentsOne = LogmanFinalOutput + "\n\n"+ "Disk Read Latency(ms) Information :" + "\n\n" + DiskReadOutput + "\n\n" + "Disk Write Latency(ms) Information :" + "\n\n" + DiskWriteOutput
            CommentsTwo = "\n\n" + "Average Disk Read and Write Latency(ms) Information :" + "\n\n" + AverageDiskRedandWriteOutput
            
            CommentsOutput = CommentsOne + CommentsTwo + HealthCheckComments
            ...
            if len(SnowSysId) > 0:
                ...
                logger.info("\n" + CommentsOutput)
                logger.info("\n" + "Message : Script Execution Successfull, unable to update comments to the Snow i.e Invalid SysId or SysId is empty")
        except:
            pass
    except:
        pass